...
public class BoardDevolver extends JPanel{
	...
	private void initBoard() {
		...
		painel1.setBounds(0, 0, 390, 250);
		painel2=new JPanel();
		painel2.setLayout(new GridLayout(1, 1));
		painel2.setBorder(BorderFactory.createTitledBorder("Lista de Leitores"));
		painel2.setBounds(390, 0, 390, 250);
		String []colunas= {"id","nome","autor","descricao","ano","categoria"};
		//livros disponiveis
		String query="select distinct L.id,L.nome,L.autor,L.ano,L.categoria,L.descricao from livro as L,exemplar as E where L.id=E.id_livro and E.estado=1";
		...
		tabela2.getSelectionModel().addListSelectionListener(new ListSelectionListener() {
			...
			public void valueChanged(ListSelectionEvent e) {
				...
				String query="select distinct L.id,L.nome,L.autor,L.ano,L.categoria,L.descricao "
						+ "from livro as L,exemplar as E,leitor as Le,emprestimo as Em "
						+ "where L.id=E.id_livro and E.estado=1 and E.id=Em.id_exemplar and Em.id_leitor="+tabela2.getValueAt(tabela2.getSelectedRow(), 0).toString();
				ArrayList<Livro> disponivel=LivroDAO.getResult(query);
				Object [][] objetos= Tools.getListObjects(disponivel,6);
				tabela=new JTable(objetos,colunas);
				barra=new JScrollPane(tabela);
		...
		realizarE=new JButton("Devolver Livro");
	...
	public static void main(String[] args) {
		JFrame j=new JFrame();
		j.getContentPane().add(new BoardDevolver());
